...
function createConfig(options) {
  const config = {
    elements: [import_path.default.join(rootDir, "elements.json")],
    rules: {
...
function canResolve(pkg3) {
  try {
    require.resolve(pkg3);
    return true;
  } catch (err) {
    return false;
  }
}
var haveVoidContent = import_html_validate.ruleExists && (0, import_html_validate.ruleExists)("void-content") || canResolve("html-validate/build/rules/void-content") || canResolve("html-validate/dist/rules/void-content");
var useLegacyVoid = !haveVoidContent;
...
var PreferSlotShorthand = class extends import_html_validate3.Rule {
  documentation(context) {
    const slot = (context == null ? void 0 : context.slot) ?? "my-slot";
    return {
      description: [
...
function haveGetMetaFor(context) {
...
function loadComponentMeta(context, node) {
  let src = node;
  if (node.tagName === "template") {
    src = src.parent;
  }
  const attr = src.getAttribute(node.meta.component);
...
function processElement(node) {
  ...
  if (node.meta && node.meta.component) {
...
function doVersionCheck(peerDependency, version) {
  ...
  if (!(0, import_semver.satisfies)(version || "0.0.0", peerDependency)) {
    console.error(
      [
        "-----------------------------------------------------------------------------------------------------",
